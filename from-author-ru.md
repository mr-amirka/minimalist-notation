...
Затем я задумался над разработкой своей технологии, предполагающей генерацию стилей на основании нотации имен классов в самой разметке, которая позволила бы ещё более автоматизировать процесс разработки и улучшить качество кода.


Главная идея: генерация стилей происходит автоматически на основе значений атрибутов,
найденных в самой разметке - ничего лишнего!


Таким образом, у Нас не будет неиспользуемого CSS-кода, и в большинстве случаев мы можем вообще не писать CSS-код,
и при желании в рантайм версии Мы можем даже не обращаться за CSS-файлами на сервер - CSS распаковывается, подобно rar-архиву, из самой разметки.  


За основу взят принцип формирования имени класса от атрибута стиля – это, по сути, аналог инлайновых стилей,
только более минималистичный, ибо я старался, сократить названия классов до аббревиатур.  


Часть имени класса отвечает за атрибут стиля, часть – за его значение.  
Однако для адаптивности этого недостаточно, поэтому я сделал ещё больше, и Мы можем увидеть это из примеров приведенных в настоящей документации.
...
Мой подход позволяет уменьшить написание кода, систематизировать именование классов и улучшить понимание их назначения в разметке.


Согласитесь, Нам в любом случае приходится писать в разметке имена классов, и если MN с чем-то сравнивать, то например:

  1. Суммарное значение атрибута ``` class ``` в Вашей разметке может быть даже меньше, чем длинные имена классов BEM.
