...
Не вижу блага в том, чтобы засорять проект файлами с лишним CSS-кодом, ибо он может сильно разрастаться, увеличивая когнитивную нагрузку. Также нам необходимо следить за тем, чтобы в проекте не накапливался CSS код, который утратил актуальность в связи с изменением разметки для которой он писался. Иногда разработчик "по-запарке" случайно удалив нечто нужное может сломать проект.
...
Часть имени класса отвечает за атрибут стиля, часть – за его значение - для адаптивности этого недостаточно, поэтому я сделал ещё больше, и Вы можете увидеть это из примеров приведенных выше.


Дабы проработать все нюансы я детальнее изучил известный мне существующий популярный на тот момент подход в разработке интерфейсов: "методологию BEM" - с которой познакомился ранее в 2017 году. BEM предлагает уникальное именование компонентов и их составляющих с целью избежания коллизий имен из разных компонентов, иерархию размещения исходников компонентов, которая по сути подобна оной в методологии компонентного подхода.  

Однако, мой подход позволяет также уменьшить написание лишнего кода, систематизировать именование классов и улучшить понимание их назначения в разметке.
...
4. Программная манипуляция глобальными стилями CSS в рантайме.  Из этого перечня манипуляция глобальными стилями CSS в рантайме используется наиболее часто в основном как способ динамической кастомизации и альтернативного задания стилей, когда мы хотим совсем не видеть CSS-файлы в своем проекте.
